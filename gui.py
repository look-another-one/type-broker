...
from capture import text_capture , shot_captcha
from google_captcha import solved_text, captcha_sender, get_solved_text
...
def gui(image,background_image_path=None):
    ...
    solve_cp = ttk.Button(frame, text="Solve Captcha", command=lambda: shot_captcha())
    solve_cp.grid(row=2, column=0, pady=10, sticky="ew")

    # Type Captcha button
    type_captcha_btn = ttk.Button(frame, text="Type Captcha", command=lambda: types_the_text(get_solved_text()))  # make sure you have a function named type_captcha()
    type_captcha_btn.grid(row=3, column=0, pady=10, sticky="ew")
